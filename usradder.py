...
def get_room_number():
    while True:
        try:
            num = int(input("Введите задержку между инвайтингом:"))
            return num
        except ValueError:
...
print(f"Инвайтинг будет с задержкой {room_number} секунд")
...
for user in users:
    ...
    try:
        ...
        print(f'{sleep}{g} Sleep {room_number}s{rs}')
        time.sleep(room_number)
